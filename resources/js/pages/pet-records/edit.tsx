...
import { update, index } from '@/routes/pet-records';

interface PetRecordEditProps {
    pet: Pet;
    petRecord: PetRecord;
}
...
export default function PetRecordEdit({ pet, petRecord }: PetRecordEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        visit_date: petRecord.visit_date,
        type_of_visit: petRecord.type_of_visit,
        description: petRecord.description,
        vet_name: petRecord.vet_name,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(update({ pet: pet.id, pet_record: petRecord.id }).url);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Record - ${pet.name}`} />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Update veterinary record for {pet.name}
                        </p>
                    </div>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Update the details of the veterinary visit
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
